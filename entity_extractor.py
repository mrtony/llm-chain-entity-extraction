...
arg_parser = argparse.ArgumentParser(
    prog="LLM CT Entity Extractor",
    description="Extracts entities from clinical text using LLMs.",
)
arg_parser.add_argument(
    "-a",
    "--api",
    default=DEFAULT_MODEL,
    help="the API to use (options: llama, openai, bard)",
)
arg_parser.add_argument(
    "--model",
    help="the model to run, dependent upon API choice. Path to model in Llama2.0, model name in OpenAI, or model name in BARD.",
)
arg_parser.add_argument(
    "--sentences",
    help="path to a set of free text clinical sentences to encode with SNOMED CT.",
)
...
if llm_api == "llama":
    ...
    raise NotImplemented(
        f"Please set the api argument to one of (llama, openai, bard). Got {llm_api}."
    )
...
NO_MATCH, DIRECT_MATCH, SIMPLIFIED_MATCH, GENERALISED_MATCH, RESPELLED_MATCH = (
    "NO MATCH",
    "DIRECT",
    "SIMPLIFIED",
    "GENERALISED",
...
def display_color(line, entities, time_taken):
    ...
    for key, value in entities.items():
        ...
        print(
            f'{color}{key}{COLOR_RESET}: {COLOR_BLUE}{match["display"]} |{match["code"]}| {color}(confidence {rating}){COLOR_RESET} {method_info}'
        )
...
server_url = "https://snowstorm-lite.nw.r.appspot.com/fhir"
...
def match_snomed(term, context=None):
    ...
    if fhir_response and "expansion" in fhir_response:
        ...
        if (
            "contains" in fhir_response["expansion"]
            and len(fhir_response["expansion"]["contains"]) > 0
        ):
            ...
            if not best_match:
                if len(list_of_matches) == 1:
                    ...
                    best_match = select_most_similar(
        ...
        print(
            ...
            "\n",
            COLOR_RED,
            "Unable to get fhir response. Using blank codes.",
            COLOR_RESET,
            sep="",
        )
...
def select_most_similar(term, list_of_matches):
    ...
    select_best_prompts[-1]["content"] = (
        "Clinician's term: {}\nPossible matches from SNOMED: {}".format(
            term, comma_separated_list_of_names
        )
    )
...
def rate(term, match, context):
    ...
    accuracy_prompts[-1]["content"] = (
        'Clinician\'s term: "{}"\nSNOMED term: "{}"\nContext: "{}"'.format(
            term, match, context
        )
    )
    response = create_chat_completion(
        accuracy_prompts, max_tokens=max_tokens_for_rating
    ).strip()
    if response in ("1", "2", "3", "4", "5"):
        return int(response)
    elif string := extract_first_digit_in_range_final(
        response
    ):  # match := re.match('[1-5](\.\d)?', response):
...
def identify_language(lines):
    ...
    return (
        create_chat_completion(
            from_prompt(language_prompts, text_extract), max_tokens=16
        )
        .strip()
        .lower()
    )
...
def as_english(text, language_description):
    ...
    if "en" in response:
        ...
        print(
            f"Language: {response} (unexpected response; {COLOR_BLUE}translating any language -> en{COLOR_RESET})"
        )
    return text
...
def identify(text):
    ...
    for term in response_terms:
        ...
        simple_term = create_chat_completion(
            from_prompt(simplify_prompts, term), max_tokens=16
        )
        ...
        general_term = create_chat_completion(
            from_prompt(generalise_prompts, term), max_tokens=16
        )
...
def main():
    ...
    if _args.sentences:
        ...
        for line in lines:
            ...
            text = as_english(
                line, language
            )  # translate text in other languages to english
        ...
        raise ValueError(
            "Please provide a path to a set of free text clinical sentences to encode with SNOMED CT."
        )
